...
def load_data(dataset, bfs_level=3, relabel=False):
    if dataset in ['WN18RR', 'FB15k-237', 'YAGO', 'WN18', 'FB15k', 'Nations']:
...
class RGCNLinkDataset(object):
    ...
    def load(self):
        ...
        if self.name not in ['WN18RR', 'FB15k-237', 'YAGO', 'WN18', 'FB15k']:
            entity_dict = _read_dictionary1(entity_path)
            relation_dict = _read_dictionary1(relation_path)
        else:
...
def _read_dictionary1(filename):
    d = {}
    with open(filename, 'r+') as f:
        for line in f:
            line = line.strip().split('\t')
